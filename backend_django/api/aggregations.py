"""
MongoDB aggregation pipelines - mirrors backend/services/aggregations.py.
...
def summary_sales_pipeline(fy: str, branch: str) -> list:
    """Pipeline for Summary: Sales Capture (View D)."""
    return [
...
def business_conversion_pipeline(fy: str) -> list:
    """Pipeline for Business Conversion Ratio (View C) - monthly."""
    return [
        {"$match": {"fy": fy}},
        {
            "$group": {
...
def summary_conversion_pipeline(fy: str, branch: str) -> list:
    """Pipeline for Summary: Conversion Ratio (View E) - per CRE/RM × proposal type."""
    return [
        build_match(fy, branch),
        {
            "$group": {
...
def sales_funnel_pipeline(fy: str, branch: str, extra_match: dict = None) -> list:
    """Pipeline for Sales Funnel (View B)."""
